...
void uart_putstr(USART_TypeDef*  uartn, char *str,...)
{   u16 i,j=0;
    va_list ap;
    va_start(ap,str);
    ...
    while(i)                                                                                    // 循环到发送完
    {
        while(USART_GetFlagStatus(uartn, USART_FLAG_TXE) == RESET);                                    // 等待发送完成
        //uart_putchar(uartn,USART3_TX_BUF[j]); //串口3的发送
        Usart_Sendbyte(uartn,USART3_TX_BUF[j]);
        i--;
        j++;
    }
}
