...
*@注意：对于静态信号量对象，其内存空间由编译器在编译期间分配，

*并应置于读写数据段或未初始化数据段上。

*相反，rt\u sem\u create（）函数将自动分配内存空间并初始化

*信号灯。

*

*@请参阅rt\u sem\u create（）

*

*@param sem是指向要初始化的信号量的指针。假设信号量的存储将为

*在应用程序中分配。

*

*@param name是一个指针，指向您要为信号量指定的名称。

*

*@param value是信号量的初始值。

*如果用于共享资源，则应将该值初始化为可用资源的数量。

*如果用于发出事件发生的信号，则应将该值初始化为0。

*

*@param flag是信号量标志，它决定了多个线程等待的排队方式
...
*RT\u IPC\u FLAG\u优先级挂起的线程将按优先级顺序排队。

*

*RT\u IPC\u FLAG\u FIFO挂起的线程将以先进先出的方法排队

*（也称为先到先得（FCFS）调度策略）。

*

*注：RT\U IPC\U FLAG\U FIFO是一种非实时调度模式。强烈建议

*使用RT\u IPC\u FLAG\u PRIO确保线程是实时的，除非您的应用程序担心
...
*@return返回操作状态。当返回值为RT\U EOK时，初始化成功。

*如果返回值是任何其他值，则表示初始化失败。
...
*@注意此函数用于分离由rt\u sem\u init（）函数初始化的静态信号量对象。

*相反，rt\u sem\u delete（）函数将删除信号量对象。

*成功分离信号量后，它将恢复信号量列表中所有挂起的线程。

*

*@请参阅rt\u sem\u delete（）

*

*@param sem是指向要分离的信号量对象的指针。

*

*@return返回操作状态。当返回值为RT\U EOK时，初始化成功。

*如果返回值是任何其他值，则表示信号量分离失败。

*

*@警告此函数只能分离由rt\u sem\u init（）函数初始化的静态信号量。

*如果信号量是由rt\u sem\u create（）函数创建的，则不能使用此函数将其分离，

*仅使用rt\u sem\u delete（）函数完成删除。
...
*@brief此函数将删除信号量对象并释放内存空间。

*

*@注意此函数用于删除由rt\u sem\u create（）函数创建的信号量对象。

*相反，rt\u sem\u detach（）函数将分离静态信号量对象。

*成功删除信号量后，它将恢复信号量列表中所有挂起的线程。

*

*@请参阅rt\u sem\u detach（）

*

*@param sem是指向要删除的信号量对象的指针。

*

*@return返回操作状态。返回值为RT\U EOK时，操作成功。

*如果返回值是任何其他值，则表示信号量分离失败。

*

*@警告此函数只能删除由rt\u sem\u create（）函数初始化的信号量。

*如果信号量是由rt\u sem\u init（）函数初始化的，则不能使用此函数删除它，

*仅使用rt\u sem\u detach（）函数完成分离。
...
*@brief此函数将接收信号量，如果信号量不可用，线程将等待

*指定时间之前的信号量。

*

*@注意：调用此函数时，sem->值的计数值将减少1，直到等于0。

*当sem->值为0时，表示信号量不可用。此时，它将暂停

*线程正在准备接收信号量。

*相反，rt\u sem\u release（）函数会将sem->value的计数值每次增加1。
...
*@param sem是指向信号量对象的指针。

*

*@param time是一个超时时间（单位：操作系统刻度）。如果信号量不可用，线程将等待

*信号量达到参数指定的时间量。

*注意：通常，我们使用宏RT\u WAITING\u FOREVER来设置此参数，这意味着当

*信号量不可用，线程将永远等待。

*

*@return返回操作状态。只有当返回值为RT\U EOK时，操作才成功。

*如果返回值是任何其他值，则表示信号量获取失败。

*

*@警告此函数只能在线程上下文中调用。不能在中断上下文中调用它。
...
rt_err_t rt_sem_take(rt_sem_t sem, rt_int32_t time)
{
    ...
    /* parameter check */
    RT_ASSERT(sem != RT_NULL);
    //rt_kprintf("aaaaaaaaaaaaaaaaaass");
    RT_ASSERT(rt_object_get_type(&sem->parent.parent) == RT_Object_Class_Semaphore);
...
/**
*@brief此函数将设置信号量对象的一些额外属性。
*
*@note目前该函数只支持RT_IPC_CMD_RESET命令来重置信号量。
*
*@param sem是指向信号量对象的指针。
*
*@param cmd是一个命令字，用于配置信号量的某些属性。
*
*@param arg是执行命令的函数的参数。
*
*@return返回操作状态。当返回值为RT_EOK时，操作成功。
*如果返回值是任何其他值，则表示此函数无法执行。
*/
